...
Original file is located at
    https://colab.research.google.com/drive/1dJnpVsPrCSD2611_DZYhUQK06302du5i

> This is the exhaustive solution of Task 2 from the LittleBank Case Study. The main goal is to predict the outcome of the marketing campaign based on the available data. Several Machine Learning algorithms are implemented to solve the task. In addition, feature importance figures are drawn to analyse the decision of each model.

> A concise solution was implemented based on this file. It is the go-to solution for the real-time interview.
"""
...
model_log_regr = LogisticRegression(C = 0.98, solver = "lbfgs", n_jobs = -1, max_iter = 500)
#model_log_regr = clf.best_estimator_

model_log_regr.fit(train_inputs, train_targets)

pd.DataFrame({
    "features": train_inputs.columns,
    "weights": model_log_regr.coef_.tolist()[0]
});

train_preds = model_log_regr.predict(train_inputs)

train_acc = accuracy_score(train_targets, train_preds)

train_acc

test_preds = model_log_regr.predict(test_inputs)
test_acc = accuracy_score(test_targets, test_preds)

test_acc

confusion_matrix_tr = confusion_matrix(train_targets, train_preds, normalize = "true")

sns.heatmap(confusion_matrix_tr, annot = True)
plt.xlabel("Predictions")
plt.ylabel("Target")
plt.title("Train Confusion Matrix")
plt.show()

recall_train = recall_score(train_targets, train_preds)#, pos_label=True")
precision_train = precision_score(train_targets, train_preds)#, pos_label="True")

recall_train, precision_train

recall_test = recall_score(test_targets, test_preds)#, pos_label = "True")
precision_test = precision_score(test_targets, test_preds)#, pos_label = "True")
...
def calc_and_plot_confusion_matrix(targets, inputs, model , name = ""):
    predictions = model.predict(inputs)
    conf_matrix = confusion_matrix(targets, predictions, normalize = "true")
...
confusion_matrix_test = calc_and_plot_confusion_matrix(test_targets, test_inputs, model_log_regr, "Test")
...
# model_decision_tree_pruned = DecisionTreeClassifier(criterion = "gini",
#                                                     max_depth = 26,           # 15-30  # The Best: 29, 26
#                                                     min_samples_split = 10,   # 2-15   # The Best: 2, 10
#                                                     min_samples_leaf = 4,     # 1-15   # The Best: 4
#                                                     #max_features = ,
...
# %%time
# path = model_decision_tree_pruned.cost_complexity_pruning_path(train_inputs, train_targets)
...
# for alpha in alphas:
#     model_decision_tree_pruned.set_params(ccp_alpha = alpha)
#     model_decision_tree_pruned.fit(train_inputs, train_targets)

#     train_preds_dtree = model_decision_tree_pruned.predict(train_inputs)
#     train_acc_temp = accuracy_score(train_targets, train_preds_dtree)

#     test_preds_dtree = model_decision_tree_pruned.predict(test_inputs)
#     test_acc_temp = model_decision_tree_pruned.score(test_inputs, test_targets)

#     f1_score_dtree_temp = f1_score(test_targets, test_preds_dtree)

#     if f1_score_dtree_temp > f1_score_best:      # f1_score_dtree_temp > f1_score_best; test_acc_temp > test_acc_best
#         test_acc_best = test_acc_temp
#         f1_score_best = f1_score_dtree_temp
#         best_alpha = alpha
#         train_acc_best = train_acc_temp

# print(f"Best Accuracy: {test_acc_best:.5f}, Best Alpha: {best_alpha}")
# print(f"Best F1 Score: {f1_score_best:.5f}")

# test_acc_temp, recall_test_temp, precision_test_temp = calculate_basic_metrics(test_targets, test_preds_dtree)

# print(f"Train Accuracy = {train_acc_best:.5f}, Test Accuracy = {test_acc_best:.5f}")
# print(f"Recall = {recall_test_temp:.5f}, Precision = {precision_test_temp:.5f}")

# model_decision_tree_pruned.tree_.max_depth

# model_decision_tree_pruned.get_n_leaves()

# importance_df = pd.DataFrame({
#     "feature": train_inputs.columns,
#     "importance": model_decision_tree_pruned.feature_importances_
# }).sort_values("importance", ascending = False)
...
models_file_dir = Path("models/")

if models_file_dir.is_dir():
    print(f"{models_file_dir} directory exists.")
else:
    print(f"{models_file_dir} does not exist. Creating one...")
    models_file_dir.mkdir(parents = True, exist_ok = True)

joblib.dump(value = best_model_pipeline, filename = "models/model_random_forest.joblib")
...
print(f"Train Accuracy = {train_acc:.5f}, Test Accuracy = {test_acc:.5f}")
print(f"Recall = {recall_test:.5}, Precision = {precision_test:.5}")

"""### 10.3 Pickle Save and Load"""
